...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage }) => {
  ...
  return (
    <div className="relative">
      ...
      {/* 手机端底部导航栏 */}
      <nav className="lg:hidden fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center py-2 px-4 z-49">
        {navItems.map(item => (
          <button
            key={item.id}
            onClick={() => setActivePage(item.id)}
